...
for filename in glob.glob(r"C:\Users\szendehrooh\Downloads\fintech_plataform_ie\fintech_plataform_ie\database\Rating_*.xlsx"):
#    connres.commit()
...
skplt.metrics.plot_roc_curve(np.array(y),fitted_model.predict_proba(X))
plt.show()
print("Accuracy: {0}".format(accuracy_score(y_pred,y)))
###cut off 0.9
good_risk = fitted_model.predict_proba(X)[:,1]
...
skplt.metrics.plot_confusion_matrix(y,new_cut,normalize=True)
plt.show()
